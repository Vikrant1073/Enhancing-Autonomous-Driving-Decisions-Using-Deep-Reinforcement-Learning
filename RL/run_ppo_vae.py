...
from models.ppo_vae_model import create_ppo_vae_model
...
env = DummyVecEnv([lambda: Monitor(AirSimCarEnv(ip_address="127.0.0.1", image_shape=(1, 84, 84), waypoints=waypoints, exploration_prob=0.5, exploration_final_prob=0.01, exploration_decay_steps=1000))])

# Create the PPO + VAE model using function imported from the model script
model, vae_model = create_ppo_vae_model(env, log_dir)

# Setting up the evaluation environment and callback (frequency is after every 100 timsteps)
eval_env = DummyVecEnv([lambda: Monitor(AirSimCarEnv(ip_address="127.0.0.1", image_shape=(1, 84, 84), waypoints=waypoints, exploration_prob=0.5, exploration_final_prob=0.01, exploration_decay_steps=1000))])
eval_callback = EvalCallback(eval_env, best_model_save_path='./logs/ppo_vae_best_model',
                             log_path='./logs/ppo_vae_eval', eval_freq=100,
...
for step in range(total_steps):
    ...
    latent_rep = latent_rep.cpu().numpy()  # Converts back to numpy for the PPO model

    # Feeds the latent representation into the PPO model
...
# Saves the model
model.save("ppo_vae_airsim_car_policy")
