...
# Create the PPO model using function imported from the model script
model = create_ppo_model(env, log_dir)

# Setting up the evaluation environment and callback (frequency is after every 100 timsteps)
eval_env = DummyVecEnv([lambda: Monitor(AirSimCarEnv(ip_address="127.0.0.1", image_shape=(1, 84, 84), waypoints=waypoints, exploration_prob=0.5, exploration_final_prob=0.1, exploration_decay_steps=5000))])
eval_callback = EvalCallback(eval_env, best_model_save_path='./logs/ppo_best_model',
                             log_path='./logs/ppo_eval', eval_freq=100,
...
for step in range(total_steps):
    ...
    # Getting the Q-values for the current observation
    q_values = model.q_net(torch.tensor(obs).float().to(model.device)).detach().cpu().numpy()

    # Logging the Q-values for each action
    for i, q_value in enumerate(q_values[0]):
        writer.add_scalar(f'QValues/Action_{i}', q_value, step)
